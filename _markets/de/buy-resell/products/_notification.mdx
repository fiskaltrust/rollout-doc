...
| 2\. Enabling Notifications | \- Go to Notifications page in Dealer Portal.  <br>\- Set Operator’s notification status to Enabled (this sends onboarding email). | —   |
...
| 5\. Notification Sending & Status Tracking | \- Track notification status (scheduled, sent, failed, blocked).  <br>\- Enable/disable or trigger notifications manually.  <br>\- Use filters to prioritize Operators.  <br>\- Keep records for compliance and support. | \- Monitor confirmation or follow-up emails.  <br>\- Stay informed if notifications are paused or disabled. |
...
![Notification Flow](./images/NotificationFlow_DE.png)## Feature Overview
