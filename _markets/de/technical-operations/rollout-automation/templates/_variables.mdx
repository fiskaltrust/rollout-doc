...
Provides a key-value pair which is used to filter the SCU of the indicated SCU type for. The scope is always your **account** and the **outlet** where you create the CashBox. You cannot reference SCUs from other outlets or accounts.
...
Let's say you have purchased fiskaly TSEs for all your outlets via fiskaltrust.Shop. In that case their SCUs should all have a `counter` field with the value of `1`. By using a reference variable in your template, you can now reference these SCUs (in particular their IDs, in this example) by using the following variable.


```
|[reference_scu_fiskaly_counter_1_id]|
```

A different example, let's assume you assign a field `masterPOS` manually to all your SCUs and wanted to reference the URL of the SCU where `masterPOS` is set to `true`. In that case you'd use the following variable.

```
|[reference_scu_fiskaly_masterPOS_true_url]|
```