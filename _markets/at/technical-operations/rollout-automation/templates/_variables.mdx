...
Where the four dynamic parts (denoted by curly brackets) specify which attribute of which component you exactly want to reference. In the second reference where the reference doesn't contain a key/value, the first existing SCU is selected.
...
Any TSE bought via fiskaltrust.Shop or 'Rollout Management' can have an incrementing integer field `counter` assigned.

:::

Let's say you have purchased two onlinescu TSEs for one outlet via fiskaltrust.Shop. In that case, their SCUs can have a `counter` field with the value of `1` and '2'. Using a reference variable in your template, you can now reference these SCUs (in particular their IDs, in this example) by using the following variable.
